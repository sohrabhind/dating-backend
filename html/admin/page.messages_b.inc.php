<?php
    ...
    if (isset($_GET['chatId'])) {
        ...
        if ($chatD["fromUserAL"] == 1) {
            $chatFromUserId = $chatD["fromUserId"];
            $chatToUserId = $chatD["toUserId"];
            $accessToken = "F43E149C4AB2B27D58A7E2D65EFF6EED";
        } elseif ($chatD["toUserAL"] == 1) {
            $chatFromUserId = $chatD["toUserId"];
            $chatToUserId = $chatD["fromUserId"];
            $accessToken = "F43E149C4AB2B27D58A7E2D65EFF6EED";
        }
    } else {
...
function sendMsg() {
    ...
    $.ajax({
        type: 'POST',
        url: '/api/v2/method/msg.new',
        data: 'accountId='+'<?php echo $chatFromUserId; ?>'+
              '&accessToken='+'<?php echo $accessToken; ?>'+
              '&profileId='+'<?php echo $chatToUserId; ?>'+
              '&chatFromUserId='+'<?php echo $chatFromUserId; ?>'+
              '&chatToUserId='+'<?php echo $chatToUserId; ?>'+
            ...
            window.alert("Error");
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="form-row">
                                    <textarea class="form-control" id="messageText" name="messageText" onkeydown="if (event.key == 'Enter' && event.ctrlKey){sendMsg(); return false;}"></textarea>
                                    <button class="form-control mt-3" type="submit" onclick="sendMsg(); return false;" id="send-btn">Send</button>
                                </div> 
                            </div>

                                    <div class="card-header">
                                        <h4 class="card-title m-b-0">Messages</h4>
                                    </div>
                                    <div class="card-body collapse show">

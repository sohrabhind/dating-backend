...
class image extends db_connect
{
    ...
    public function commentsCreate($imageId, $text, $notifyId = 0, $replyToUserId = 0)
    {
        ...
        $imageInfo = $images->info($imageId);

        $currentTime = time();
        $ip_addr = helper::ip_addr();

        $stmt = $this->db->prepare("INSERT INTO images_comments (fromUserId, replyToUserId, imageId, comment, createAt, notifyId, ip_addr) value (:fromUserId, :replyToUserId, :imageId, :comment, :createAt, :notifyId, :ip_addr)");

...
class photos extends db_connect
{
    ...
    {
        $sql = "SELECT count(*) FROM photos WHERE fromUserId = {$this->requestFrom} AND removeAt = 0";
    ...
    public function get($profileId, $photoId = 0, $accessMode = 0, $itemType = -1, $limit = 20)
    {
        if ($photoId == 0) {
        ...
        if ($accessMode == 0) {
            ...
            if ($itemType != -1) {
                ...
                $stmt = $this->db->prepare("SELECT id FROM photos WHERE accessMode = 0 AND fromUserId = (:fromUserId) AND itemType = (:itemType) AND removeAt = 0 AND id < (:photoId) ORDER BY id DESC LIMIT :limit");
                ...
                $stmt = $this->db->prepare("SELECT id FROM photos WHERE accessMode = 0 AND fromUserId = (:fromUserId) AND removeAt = 0 AND id < (:photoId) ORDER BY id DESC LIMIT :limit");
                $stmt->bindParam(':fromUserId', $profileId, PDO::PARAM_INT);
                $stmt->bindParam(':photoId', $photoId, PDO::PARAM_INT);
                $stmt->bindParam(':limit', $limit, PDO::PARAM_INT);
            }
            ...
            if ($this->getRequestFrom() == $profileId) {
                ...
                if ($itemType != -1) {
                    ...
                    $stmt = $this->db->prepare("SELECT id FROM photos WHERE fromUserId = (:fromUserId) AND itemType = (:itemType) AND removeAt = 0 AND id < (:photoId) ORDER BY id DESC LIMIT :limit");
                    $stmt->bindParam(':fromUserId', $profileId, PDO::PARAM_INT);
                    $stmt->bindParam(':itemType', $itemType, PDO::PARAM_INT);
                    $stmt->bindParam(':photoId', $photoId, PDO::PARAM_INT);
                    $stmt->bindParam(':limit', $limit, PDO::PARAM_INT);
                    ...
                    $stmt = $this->db->prepare("SELECT id FROM photos WHERE fromUserId = (:fromUserId) AND removeAt = 0 AND id < (:photoId) ORDER BY id DESC LIMIT :limit");
                    $stmt->bindParam(':fromUserId', $profileId, PDO::PARAM_INT);
                    $stmt->bindParam(':photoId', $photoId, PDO::PARAM_INT);
                    $stmt->bindParam(':limit', $limit, PDO::PARAM_INT);
                }
